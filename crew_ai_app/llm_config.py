...
def get_deepseek_llm_config(
    model_id: Optional[str] = None,
    temperature: float = 0.1,
    max_tokens: int = 4096
) -> Dict[str, Any]:
    """Get DeepSeek LLM configuration for CrewAI agents.
    ...
    model_id = model_id or os.getenv("DEEPSEEK_MODEL_ID", "deepseek-chat")
    
    # LiteLLM format for DeepSeek
    litellm_model = f"deepseek/{model_id}"
    
    logger.info(f"Configuring DeepSeek LLM: {litellm_model}")
...
def get_llm_config(
    ...
    if provider == "deepseek":
        return get_deepseek_llm_config(model_id, temperature, max_tokens)
    else:
        return get_bedrock_llm_config(model_id, None, temperature, max_tokens)
